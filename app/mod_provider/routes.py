...
import logging
from app.mod_provider.forms import DateForm, ServiceForm, AddServiceForm, BookDateForm, BookTimeForm, EditShopProfile, UpdateShopPassword
...
def profile():
	...
	my_profile= Shop.query.filter_by(shopId= current_user.shopId).first()
...
def edit_profile():
	...
	User_profile= User.query.filter_by(id= current_user.id).first()
	profile= Shop.query.filter_by(shopId= current_user.shopId).first()
	if form.validate_on_submit():
		print("validated")
		profile.shopname=form.shopname.data
		...
		User_profile.phonenumber= form.phonenumber.data
		db.session.commit()
	...
	if request.method== 'GET':
		form.shopname.data = profile.shopname
		...
		form.phonenumber.data= User_profile.phonenumber

	

	return render_template('provider/edit_profile.html',form=form, profile= profile,User_profile=User_profile, title= "Update Profile")
...
def update_password():
	...
	return render_template('provider/update_password.html',form=form,form_action=form_action, profile= profile, title= "Update Password")
